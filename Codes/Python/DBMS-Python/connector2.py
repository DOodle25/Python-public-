...
mycursor.execute("USE tables_practice_1")
mycursor.execute("SELECT * FROM restart")
tables1 = mycursor.fetchall()
print("table 1:")
print_list_as_sql_table(tables1)
mycursor.execute("SELECT * FROM CLOUD_PRACTITIONER")
tables2 = mycursor.fetchall()
print("table 2")
print_list_as_sql_table(tables2)

#join
#mycursor.execute("SELECT e.id, e.name, d.certification_date, e.graduation_date FROM restart e INNER JOIN CLOUD_PRACTITIONER d ON e.id = d.student_id;")
mycursor.execute("")
